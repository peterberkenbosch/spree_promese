class Promese::StockDeserializer < PromeseDeserializer
  ...
  def unprocessed_stock(stock_item)
    ...
    Spree::LineItem.joins(:variant, :order).where(spree_orders: {promese_exported: true, promese_processed_at: nil}, spree_variants: {sku: stock_item['product_id']}).sum(:quantity)
  end
